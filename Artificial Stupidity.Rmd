...
```{r}
# corData <- cor(allData %>% select(!is.character))
# corData %>% as_tibble() %>% rownames_to_column() %>% gather(colname, value, -rowname) %>% ggplot(aes(x = rowname, y = colname, fill = value)) + geom_tile()

# beforeCluster <- allData %>% select(!is.character)
# 
# db <- dbscan(beforeCluster %>% select(-price), eps = 50, minPts = 3)
# db
