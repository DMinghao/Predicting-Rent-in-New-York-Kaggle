...
allData <- allData %>% merge(mean_price, by = c("neighbourhood_cleansed", "neighbourhood_cleansed"))
...
# boruta_output <- Boruta(price ~ ., data=trainData, pValue = 0.05, maxRuns = 11) 
# selectedCols <- boruta_output %>% getSelectedAttributes(withTentative = TRUE)
# 
# trainData <- trainData %>% select(c(price, boruta_output %>% unlist()))
# testData <- testData %>% select(c(price, boruta_output %>% unlist()))
