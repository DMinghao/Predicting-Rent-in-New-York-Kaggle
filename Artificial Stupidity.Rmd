...
# conda_install("r-reticulate", "tensorflow-gpu")
...
model <- keras_model_sequential() %>%
  layer_dense(units = dim(NetTrainX)[2]*2, activation = "softplus", input_shape = dim(NetTrainX)[2]) %>%
  layer_dropout(rate = 0.33) %>%
  layer_dense(units = dim(NetTrainX)[2]*4, activation = "softplus") %>%
  layer_dropout(rate = 0.33) %>%
  layer_dense(units = dim(NetTrainX)[2]*8, activation = "softplus") %>%
  layer_dropout(rate = 0.33) %>%
  layer_dense(units = dim(NetTrainX)[2]*4, activation = "softplus") %>%
  layer_dropout(rate = 0.33) %>%
  layer_dense(units = dim(NetTrainX)[2]*2, activation = "softplus") %>%
  layer_dropout(rate = 0.33) %>%
  layer_dense(units = dim(NetTrainX)[2]*0.5, activation = "softplus") %>%
  layer_dense(units = dim(NetTrainX)[2]*0.25, activation = "softplus") %>%
