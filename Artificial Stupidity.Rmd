...
allData[is.na(allData$price_mean_c),]$price_mean_c = mean(allData$price_mean_c, na.rm = TRUE)
...
allData <- allData %>% merge(mean_price2, by = c("neighbourhood_group_cleansed", "neighbourhood_group_cleansed"))
...
boruta_output <- Boruta(price ~ ., data=trainData, pValue = 0.05, maxRuns = 1000)  %>% 
  getSelectedAttributes(withTentative = TRUE)
