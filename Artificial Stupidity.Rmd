...
BoostTrainData <- xgb.DMatrix(
  model.matrix(price ~ ., data = BoostTrainData),
  label = as.matrix(BoostTrainData %>% select(price)),
  base_margin = log(BoostTrainData$price)
)
...
pred <- predict(model, testData %>% select(!is.character) %>% as.matrix())
