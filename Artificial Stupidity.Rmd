...
hyper_grid <- expand.grid(max_depth = seq(2, 10, 1), eta = seq(.05, .35, .01))
...
model <- keras_model_sequential() %>%
  layer_dense(units = 128, activation = "softplus", input_shape = dim(NetTrainX)[2]) %>%
  ...
  layer_dropout(rate = 0.33) %>%
  layer_dense(units = 256, activation = "softplus") %>%
  layer_dense(units = 128, activation = "softplus") %>%
